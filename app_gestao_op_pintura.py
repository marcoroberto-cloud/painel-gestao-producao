...
def processar_todas_as_bases(mtimes, pasta_base=STORAGE_DIR):
  ...
  df_op_raw = (
      pd.read_parquet(p_op) if os.path.exists(p_op) else pd.DataFrame()
  )
  df_rom_raw = (
      pd.read_parquet(p_rom) if os.path.exists(p_rom) else pd.DataFrame()
  )
  df_comp_raw = (
      pd.read_parquet(p_comp) if os.path.exists(p_comp) else pd.DataFrame()
  )
  df_sc_raw = (
      pd.read_parquet(p_sc) if os.path.exists(p_sc) else pd.DataFrame()
  )
  ...
  if not df_op_raw.empty:
    ...
    if not col_obs_op:
      if len(df_op_raw.columns) >= 2 and "OBSERV" in normalizar_texto(
          df_op_raw.columns[1]
      ):
    ...
    col_desc_op = buscar_col_flex(
        df_op_raw, ["DESC. PROD", "DESCRICAO", "DESCRIÇÃO", "DESC PROD"]
    )
    col_qtd_op = buscar_col_flex(
        df_op_raw, ["QUANTIDADE", "QUANTI", "QTD PLAN"]
    )
    col_prodz_op = buscar_col_flex(
        df_op_raw, ["QTD.PRODUZID", "PRODUZIDO", "QTD PRODUZIDA", "QTD.PRODUZ"]
    )
    col_mes_op = buscar_col_flex(
        df_op_raw, ["MÊS-ANO", "MES-ANO", "MÊS", "MES"]
    )
    col_dt_fim = buscar_col_flex(
        df_op_raw, ["DT REAL FIM", "REAL FIM", "DATA FIM", "DATA"]
    )

    df_op = df_op_raw.copy()
    df_op["OBS_NORM"] = (
        df_op[col_obs_op].apply(normalizar_texto) if col_obs_op else ""
    )
    df_op["COD_PECA"] = (
        df_op[col_prod_op].apply(limpar_cod) if col_prod_op else ""
    )
    df_op["DESC_PECA"] = (
        df_op[col_desc_op].fillna("-").astype(str) if col_desc_op else "-"
    )
    df_op["QTD_PLAN"] = (
        df_op[col_qtd_op].apply(converter_num) if col_qtd_op else 0.0
    )
    df_op["QTD_PROD"] = (
        df_op[col_prodz_op].apply(converter_num) if col_prodz_op else 0.0
    )
    df_op["MES_ANO"] = (
        df_op[col_mes_op].astype(str).str.strip() if col_mes_op else "Geral"
    )
    df_op["ANO"] = df_op["MES_ANO"].str.extract(r"^(\d{4})")[0]
    df_op["MES"] = df_op["MES_ANO"].str.extract(r"/(\d{2})")[0]
    df_op["DT_FABR"] = (
        df_op[col_dt_fim].apply(formatar_data_br) if col_dt_fim else "-"
    )
  ...
  if not df_rom_raw.empty:
    ...
    col_prod_rom = buscar_col_flex(
        ...
        excluir_padroes=["PINTURA", "DESC", "TRATAMENTO"],
    )
    col_desc_rom = buscar_col_flex(
        df_rom_raw, ["DESCRIÇÃO", "DESCRICAO", "DESC. PROD", "DESC"]
    )
    col_qtd_rom = buscar_col_flex(
        df_rom_raw,
        ["QTD", "QTDE", "QUANTIDADE"],
        excluir_padroes=["RET", "SALDO"],
    )
    col_ret_rom = buscar_col_flex(
        df_rom_raw, ["QT RET", "QT_RET", "RETORNADO", "QTD RET", "QT"]
    )

    if col_ret_rom == col_qtd_rom:
      cols_disp = [
          c
          for c in df_rom_raw.columns
          if "QT" in normalizar_texto(c) or "RET" in normalizar_texto(c)
      ]
      if len(cols_disp) >= 2:
    ...
    col_dt_envio_rom = buscar_col_flex(
        df_rom_raw,
        [
            "DT EMISSÃO",
            "DT EMISSÃ",
    ...
    col_nf_ret_rom = buscar_col_flex(
        df_rom_raw, ["NF RET", "NF_RET", "NOTA RET", "NF RETORNO"]
    )
    col_dt_ret_rom = buscar_col_flex(
        df_rom_raw, ["DT RET", "DT_RET", "DATA RET", "DATA RETORNO"]
    )

    df_rom = df_rom_raw.copy()
    df_rom["OBS_NORM"] = (
        df_rom[col_obs_rom].apply(normalizar_texto) if col_obs_rom else ""
    )
    df_rom["COD_PECA"] = (
        df_rom[col_prod_rom].apply(limpar_cod) if col_prod_rom else ""
    )
    df_rom["DESC_PECA"] = (
        df_rom[col_desc_rom].fillna("-").astype(str) if col_desc_rom else "-"
    )
    df_rom["QTD_ENV"] = (
        df_rom[col_qtd_rom].apply(converter_num) if col_qtd_rom else 0.0
    )
    df_rom["QTD_RET"] = (
        df_rom[col_ret_rom].apply(converter_num) if col_ret_rom else 0.0
    )
    df_rom["SALDO_RUA"] = (
        df_rom[col_saldo_rom].apply(converter_num) if col_saldo_rom else 0.0
    )
    df_rom["DOC_ROMANEIO"] = (
        df_rom[col_doc_rom].fillna("-").astype(str) if col_doc_rom else "-"
    )
    df_rom["DATA_ENVIO"] = (
        df_rom[col_dt_envio_rom].apply(formatar_data_br)
        if col_dt_envio_rom
        else "-"
    )
    df_rom["NF_RETORNO"] = (
        df_rom[col_nf_ret_rom].fillna("-").astype(str) if col_nf_ret_rom else "-"
    )
    df_rom["DATA_RETORNO"] = (
        df_rom[col_dt_ret_rom].apply(formatar_data_br)
        if col_dt_ret_rom
        else "-"
    )
    df_rom["FORNECEDOR_TRAT"] = (
        df_rom[col_forn_rom].apply(padronizar_fornecedor_romaneio)
        if col_forn_rom
        else "-"
    )
  ...
  if not df_comp_raw.empty:
    col_obs_comp = (
        df_comp_raw.columns[11]
        if len(df_comp_raw.columns) >= 12
        else buscar_col_flex(
            df_comp_raw,
            [
                "OBSERVAÇÃO",
                "OBSERVAÇÕES",
    ...
    col_prod_comp = buscar_col_flex(
        df_comp_raw, ["PRODUTO", "COD PROD", "CODIGO"]
    )
    col_desc_comp = buscar_col_flex(
        df_comp_raw, ["DESCRIÇÃO", "DESCRICAO", "DESC. PROD"]
    )
    col_forn_comp = buscar_col_flex(df_comp_raw, ["FORNECEDOR"])
    col_qt_comp = buscar_col_flex(df_comp_raw, ["QT", "QUANTIDADE", "QTD"])
    col_ent_comp = buscar_col_flex(
        df_comp_raw, ["QTD ENTREGUE", "QTD.ENTREGUE", "ENTREGUE"]
    )
    col_fal_comp = buscar_col_flex(df_comp_raw, ["FAL", "FALTA"])
    col_dt_comp = buscar_col_flex(
        df_comp_raw, ["DT ENT.", "DT ENT", "DATA ENTREGA"]
    )
    col_nf_comp = buscar_col_flex(
        df_comp_raw, ["NF ENT.", "NF ENT", "NOTA FISCAL", "NF"]
    )
    col_dt_forn = buscar_col_flex(
        df_comp_raw, ["DATA FORNECEDOR", "DT FORNECEDOR"]
    )

    df_comp = df_comp_raw.copy()
    df_comp["OBS_NORM"] = (
        df_comp[col_obs_comp].apply(normalizar_texto) if col_obs_comp else ""
    )
    df_comp["COD_PECA"] = (
        df_comp[col_prod_comp].apply(limpar_cod) if col_prod_comp else ""
    )
    df_comp["Descricao"] = (
        df_comp[col_desc_comp].fillna("-").astype(str) if col_desc_comp else "-"
    )
    df_comp["Fornecedor"] = (
        df_comp[col_forn_comp].fillna("-").astype(str) if col_forn_comp else "-"
    )
    df_comp["Qtd_Comprada"] = (
        df_comp[col_qt_comp].apply(converter_num) if col_qt_comp else 0.0
    )
    df_comp["Qtd_Entregue"] = (
        df_comp[col_ent_comp].apply(converter_num) if col_ent_comp else 0.0
    )

    if col_fal_comp:
      df_comp["Saldo_Falta_Entregar"] = df_comp[col_fal_comp].apply(
          converter_num
      )
    else:
      df_comp["Saldo_Falta_Entregar"] = (
          df_comp["Qtd_Comprada"] - df_comp["Qtd_Entregue"]
      ).clip(lower=0.0)

    df_comp["Data_Entrega"] = (
        df_comp[col_dt_comp].apply(formatar_data_br) if col_dt_comp else "-"
    )
    df_comp["NF_Entrega"] = (
        df_comp[col_nf_comp].fillna("-").astype(str) if col_nf_comp else "-"
    )
    df_comp["Data_Fornecedor"] = (
        df_comp[col_dt_forn].apply(formatar_data_br) if col_dt_forn else "-"
    )
    ...
    for _, r in df_comp[["COD_PECA", "Descricao"]].dropna().iterrows():
      ...
      if (
          c
          and d
          and d not in ["-", "NAN", "NONE", ""]
          and c not in catalogo_descricoes
      ):
        catalogo_descricoes[c] = d
  ...
  if not df_sc_raw.empty:
    ...
    col_num_sc = buscar_col_flex(
        df_sc_raw,
        ["Nº SOLICITAÇÃO", "NUM SOLICITACAO", "SOLICITACAO", "SC", "Nº SC"],
    )
    col_item_sc = buscar_col_flex(df_sc_raw, ["ITEM"])
    col_prod_sc = buscar_col_flex(
        df_sc_raw, ["PRODUTO", "COD PROD", "CODIGO"]
    )
    col_um_sc = buscar_col_flex(df_sc_raw, ["UM", "UN"])
    col_desc_sc = buscar_col_flex(df_sc_raw, ["DESCRIÇÃO", "DESCRICAO"])
    col_qtd_sc = buscar_col_flex(
        df_sc_raw, ["QTDE DA SC", "QTD SC", "QUANTIDADE", "QTD"]
    )
    col_nec_sc = buscar_col_flex(
        df_sc_raw, ["NECESSIDADE", "DT NECESSIDADE", "DATA NECESSIDADE"]
    )
    col_obs_sc = (
        df_sc_raw.columns[8]
        if len(df_sc_raw.columns) >= 9
        else buscar_col_flex(
            df_sc_raw,
            [
                "OBSERVAÇÃO",
    ...
    col_emis_sc = buscar_col_flex(
        df_sc_raw, ["EMISSÃO", "EMISSAO", "DT EMISSAO"]
    )
    col_solic_sc = buscar_col_flex(
        df_sc_raw, ["SOLICITANTE", "NOME SOLICITANTE"]
    )
    ...
    df_sc["OBS_NORM"] = (
        df_sc[col_obs_sc].apply(normalizar_texto) if col_obs_sc else ""
    )
    df_sc["COD_PECA"] = (
        df_sc[col_prod_sc].apply(limpar_cod) if col_prod_sc else ""
    )
    df_sc["Filial"] = (
        df_sc[col_filial_sc].fillna("-").astype(str) if col_filial_sc else "-"
    )
    df_sc["Num_SC"] = (
        df_sc[col_num_sc].fillna("-").astype(str) if col_num_sc else "-"
    )
    df_sc["Item"] = (
        df_sc[col_item_sc].fillna("-").astype(str) if col_item_sc else "-"
    )
    df_sc["UM"] = (
        df_sc[col_um_sc].fillna("PC").astype(str) if col_um_sc else "PC"
    )
    df_sc["Descricao"] = (
        df_sc[col_desc_sc].fillna("-").astype(str) if col_desc_sc else "-"
    )
    df_sc["Qtd_SC"] = (
        df_sc[col_qtd_sc].apply(converter_num) if col_qtd_sc else 0.0
    )
    df_sc["Necessidade"] = (
        df_sc[col_nec_sc].apply(formatar_data_br) if col_nec_sc else "-"
    )
    df_sc["Emissao"] = (
        df_sc[col_emis_sc].apply(formatar_data_br) if col_emis_sc else "-"
    )
    df_sc["Solicitante"] = (
        df_sc[col_solic_sc].fillna("-").astype(str) if col_solic_sc else "-"
    )
    df_sc["Classe_Valor"] = (
        df_sc[col_classe_sc].fillna("-").astype(str) if col_classe_sc else "-"
    )
    df_sc["Pedido"] = (
        df_sc[col_ped_sc].fillna("-").astype(str) if col_ped_sc else "-"
    )
  ...
  df_cruz_obs = pd.merge(
      op_obs, rom_obs, on=["OBS_NORM", "COD_PECA"], how="outer"
  )
  ...
  if not df_cruz_obs.empty:
    ...
    if "Descricao_Rom" in df_cruz_obs.columns:
      cond_desc_vazia = df_cruz_obs["Descricao"].isna() | df_cruz_obs[
          "Descricao"
      ].isin(["-", "", "None", "nan"])
      df_cruz_obs["Descricao"] = df_cruz_obs["Descricao"].where(
          ~cond_desc_vazia, df_cruz_obs["Descricao_Rom"]
      )
    ...
    df_cruz_obs["Saldo_Pendente_Pintura"] = (
        df_cruz_obs["Env_Pintura"] - df_cruz_obs["Ret_Pintura"]
    ).clip(lower=0.0)
    df_cruz_obs["Aguardando_Envio"] = (
        df_cruz_obs["Qtd_Fabr"] - df_cruz_obs["Env_Pintura"]
    ).clip(lower=0.0)
    df_cruz_obs["Falta_Fabricar"] = (
        df_cruz_obs["Qtd_OP"] - df_cruz_obs["Qtd_Fabr"]
    ).clip(lower=0.0)

    # Padronização de Status para usar o mesmo padrão verde de Compras
    def calc_status(r):
      if (
          r["Ret_Pintura"] >= r["Qtd_OP"]
          and r["Qtd_OP"] > 0
          and r["Saldo_Pendente_Pintura"] == 0
          and r["Falta_Fabricar"] == 0
      ):
        return "✅ 100% Entregue"
...
with col_head_up:
  ...
  with col_btn_acoes:
    ...
    opcoes_historico = ["📁 Versão Atual"] + [
        f"🕒 Backup: {h[1]}" for h in historicos
    ]

    escolha_versao = st.selectbox(
        "⏳ Histórico de Versões:", opcoes_historico, index=0
    )
      ...
      try:
        ...
        if not f.name.endswith(".csv"):
          ...
          for s_name in xl.sheet_names:
            df_sheet_head = pd.read_excel(
                io.BytesIO(file_bytes), sheet_name=s_name, nrows=5
            )
            ...
            elif "PEDIDO" in s_upper or (
                any("QTD ENTREGUE" in c for c in cols)
                and any("TAT" in c for c in cols)
            ):
              caminho_salvar = caminho_comp_pqt
            elif (
                "OP" in f_upper
                or any("PRODUZID" in c for c in cols)
                or any("CLASSE VALOR" in c for c in cols)
            ):
              caminho_salvar = caminho_op_pqt
            elif (
                "ROMANEIO" in f_upper
                or any("CLIENTE/FORN" in c for c in cols)
                or any("DOC.ORIG" in c for c in cols)
            ):
              if any("OBSERVA" in c for c in cols) or len(cols) >= 15:
                caminho_salvar = caminho_rom_pqt

            if caminho_salvar:
              df_completo = ler_excel_completo(
                  io.BytesIO(file_bytes), sheet_name=s_name
              )
          ...
          if any("SOLICITA" in c for c in cols):
            ...
            caminho_salvar = caminho_comp_pqt
          elif any("CLIENTE/FORN" in c for c in cols) or any(
              "DOC.ORIG" in c for c in cols
          ):
            caminho_salvar = caminho_rom_pqt
...
if (
    df_op_raw.empty
    and df_rom_raw.empty
    and df_comp_raw.empty
    and df_sc_raw.empty
):
  st.info(
      "👆 Nenhuma planilha salva ainda. Selecione os arquivos no campo acima"
      " para carregar o painel."
  )
  st.stop()
...
lista_todas_obs = sorted([
    str(p)
    for p in todas_obs_set
    if str(p).strip() and str(p) not in ["-", "NAN", "NONE"]
])
...
with col_multisel_obs:
  sel_obs_global = st.multiselect(
      ...
      placeholder=(
          "Digite parte do nome ou código (Ex: 11761.01G, COROLLA, RANGER)..."
      ),
  )

with col_busca_peca:
  busca_cod = st.text_input(
      "🔍 Buscar Peça:", placeholder="Código ou descrição..."
  ).strip().upper()
...
# FILTRO 100% ESTRITO PELAS OBSERVAÇÕES FLEGADAS
if sel_obs_global:
  ...
  if not df_comp_trabalho.empty and "OBS_NORM" in df_comp_trabalho.columns:
    df_comp_trabalho = df_comp_trabalho[
        df_comp_trabalho["OBS_NORM"].isin(sel_obs_global)
    ]

  if not df_sc_trabalho.empty and "OBS_NORM" in df_sc_trabalho.columns:
    df_sc_trabalho = df_sc_trabalho[
        df_sc_trabalho["OBS_NORM"].isin(sel_obs_global)
    ]

  projeto_ativo_nome = ", ".join(sel_obs_global[:2]) + (
      "..." if len(sel_obs_global) > 2 else ""
  )
else:
...
falta_fab = (
    int(df_trabalho["Falta_Fabricar"].sum()) if not df_trabalho.empty else 0
)
falta_env = (
    int(df_trabalho["Aguardando_Envio"].sum()) if not df_trabalho.empty else 0
)
saldo_rua = (
    int(df_trabalho["Saldo_Pendente_Pintura"].sum())
    if not df_trabalho.empty
    else 0
)

tot_comprado = (
    int(df_comp_trabalho["Qtd_Comprada"].sum())
    if not df_comp_trabalho.empty
    else 0
)
tot_entregue = (
    int(df_comp_trabalho["Qtd_Entregue"].sum())
    if not df_comp_trabalho.empty
    else 0
)
saldo_compra = (
    int(df_comp_trabalho["Saldo_Falta_Entregar"].sum())
    if not df_comp_trabalho.empty
    else 0
)

tot_sc_aberto = (
    int(df_sc_trabalho["Qtd_SC"].sum()) if not df_sc_trabalho.empty else 0
)
...
c1.metric(
    "1. Programado (OP)",
    f"{tot_op:,} pçs",
    f"Falta Fabr: {falta_fab:,} pçs",
    delta_color="inverse" if falta_fab > 0 else "normal",
)
c2.metric("2. Fabricado Interno", f"{tot_fab:,} pçs", f"{pct_fab:.1f}% Produzido")
c3.metric(
    "3. Enviado Tratamento",
    f"{tot_env:,} pçs",
    f"Aguardando: {falta_env:,} pçs",
    delta_color="inverse" if falta_env > 0 else "normal",
)
c4.metric(
    "4. Retornado (Pronto)",
    f"{tot_ret:,} pçs",
    f"Falta Voltar: {saldo_rua:,} pçs",
    delta_color="inverse" if saldo_rua > 0 else "normal",
)
c5.metric(
    "5. Compras Externas",
    f"{tot_entregue:,} / {tot_comprado:,} pçs",
    f"Falta: {saldo_compra:,} pçs",
    delta_color="inverse" if saldo_compra > 0 else "normal",
)
c6.metric(
    "6. SC em Aberto",
    f"{tot_sc_aberto:,} pçs",
    f"{qtd_itens_sc} solicitações",
    delta_color="inverse" if tot_sc_aberto > 0 else "normal",
)
...
with tab_mensal:
  ...
  if not df_op.empty:
    ...
    anos_disp = sorted(
        [a for a in df_op["ANO"].dropna().unique() if len(str(a)) == 4]
    )
    with col_f_ano:
    ...
    if sel_obs_global:
      df_op_filtrada_data = df_op_filtrada_data[
          df_op_filtrada_data["OBS_NORM"].isin(sel_obs_global)
      ]

    if sel_anos:
      df_op_filtrada_data = df_op_filtrada_data[
          df_op_filtrada_data["ANO"].isin(sel_anos)
      ]

    meses_disp = sorted(
        [m for m in df_op_filtrada_data["MES"].dropna().unique() if str(m).isdigit()]
    )
    with col_f_mes:
      sel_meses = st.multiselect(
          "📅 Filtrar Mês(es):", meses_disp, default=meses_disp
      )
    if sel_meses:
      df_op_filtrada_data = df_op_filtrada_data[
          df_op_filtrada_data["MES"].isin(sel_meses)
      ]

    df_mes_prod = (
        df_op_filtrada_data.groupby("MES_ANO", as_index=False)
        .agg(
    ...
    df_mes_prod = df_mes_prod[
        df_mes_prod["MES_ANO"].str.contains(r"^\d{4}", regex=True, na=False)
    ]

    tot_plan_periodo = int(df_mes_prod["Qtd_Planejada"].sum())
    tot_prod_periodo = int(df_mes_prod["Qtd_Produzida"].sum())
    eficiencia_fabril = (
        (tot_prod_periodo / tot_plan_periodo * 100)
        if tot_plan_periodo > 0
        else 0.0
    )
    ...
    df_projetos_gargalo = df_op_filtrada_data.groupby(
        "OBS_NORM", as_index=False
    ).agg(
        Planejado=("QTD_PLAN", "sum"),
        Fabricado=("QTD_PROD", "sum"),
        Qtd_Itens=("COD_PECA", "count"),
    )
    df_projetos_gargalo["Falta_Produzir"] = (
        df_projetos_gargalo["Planejado"] - df_projetos_gargalo["Fabricado"]
    ).clip(lower=0.0)
    df_pendentes_fab = df_projetos_gargalo[
        df_projetos_gargalo["Falta_Produzir"] > 0
    ].sort_values("Falta_Produzir", ascending=False)

    if not df_pendentes_fab.empty:
      with st.expander(
          f"🚨 **{len(df_pendentes_fab)} Projetos/Lotes aguardando conclusão na"
          f" fábrica ({int(df_pendentes_fab['Falta_Produzir'].sum()):,} peças"
          " pendentes)**",
          expanded=True,
      ):
        for _, r_gar in df_pendentes_fab.iterrows():
          pct_g = (
              (r_gar["Fabricado"] / r_gar["Planejado"] * 100)
              if r_gar["Planejado"] > 0
              else 0.0
          )
          st.markdown(f"""
      ...
      st.success(
          "🎉 Todos os projetos filtrados foram 100% fabricados internamente ou"
          " não possuem pendências!"
      )
    ...
    with col_g_mes:
      ...
      if not df_mes_prod.empty:
        chart_data = df_mes_prod.set_index("MES_ANO")[
            ["Qtd_Produzida", "Qtd_Planejada"]
        ]
    ...
    with col_t_mes:
      ...
      with c_m1:
        renderizar_botao_copiar_tabela(df_mes_view, "btn_cp_mensal")
      with c_m2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(
                df_mes_view, "Producao_Mensal_Consolidada"
            ),
            file_name="Producao_Mensal_Consolidada.xlsx",
            mime=(
      ...
      st.dataframe(
          df_mes_view, use_container_width=True, hide_index=True, height=350
      )
  else:
...
with tab_mobile:
  ...
  if (
    ...
    status_geral_badge = (
        '<span class="badge-status badge-ok">✅ PROJETO 100% PRONTO & LIBERADO'
        " PARA MONTAGEM</span>"
    )
  elif saldo_rua > 0:
    status_geral_badge = (
        '<span class="badge-status badge-warn">⏳ AGUARDANDO RETORNO DE PINTURA'
        f" ({saldo_rua} pçs na rua)</span>"
    )
  elif falta_fab > 0:
    status_geral_badge = (
        '<span class="badge-status badge-alert">⚙️ GARGALO NA FÁBRICA'
        f" ({falta_fab} pçs a produzir)</span>"
    )
  else:
    status_geral_badge = (
        '<span class="badge-status badge-ok">🚀 FLUXO NORMAL EM'
        " ANDAMENTO</span>"
    )
  ...
  st.caption(
      f"**Prontidão Metálica Total:** `{pct_pronto:.1f}%` ({tot_ret:,} de"
      f" {tot_op:,} peças prontas)"
  )
  ...
  if not df_trabalho.empty:
    ...
    def classificar_tipo(desc):
      d = str(desc).upper()
      if "DIVISORIA" in d:
        return "🚪 Divisórias de Cela"
      elif "ESTRIBO" in d:
        return "🪜 Estribos Laterais"
      elif "CONSOLE" in d or "ACABAMENTO" in d or "CHAPA" in d:
        return "📐 Chapas & Acabamentos"
      elif "SUP" in d or "SUPORTE" in d:
        return "🔧 Suportes & Fixações"
      elif "GRADE" in d or "VIDRO" in d:
        return "🛡️ Grades & Vidros"
      elif "BANCO" in d or "CADEIRA" in d:
        return "💺 Bancos & Assentos"
      else:
        return "🔩 Estruturas Diversas"

    df_trabalho_tipo = df_trabalho.copy()
    df_trabalho_tipo["TIPO_CONJUNTO"] = df_trabalho_tipo["Descricao"].apply(
        classificar_tipo
    )

    tipos_agg = df_trabalho_tipo.groupby(
        "TIPO_CONJUNTO", as_index=False
    ).agg(
    ...
    for i, r_tipo in tipos_agg.iterrows():
      ...
      with alvo_col:
        pct_tipo = (
            (r_tipo["Ja_Retornado"] / r_tipo["Total_Necessario"] * 100)
            if r_tipo["Total_Necessario"] > 0
            else 100.0
        )
        st.markdown(
            f"""
  ...
  if not df_trabalho.empty:
    ...
    if not df_na_rua.empty:
      for forn, group in df_na_rua.groupby("Fornecedor_Tratamento"):
        ...
        with st.expander(
            f"🔴 **{forn}**: {saldo_forn:,} peças pendentes ({len(group)}"
            " itens)"
        ):
          for _, r in group.iterrows():
            st.markdown(
                f"• **`{r['COD_PECA']}`** ({r['Descricao'][:40]}):"
                f" **{int(r['Saldo_Pendente_Pintura'])} pçs** | Romaneio:"
                f" `{r['Doc_Romaneio']}` (Envio: {r['Data_Envio']})"
            )
    else:
      st.success(
          "🎉 Nenhuma peça na rua! 100% das peças enviadas para tratamento já"
          " retornaram."
      )
  ...
  if not df_comp_trabalho.empty or not df_sc_trabalho.empty:
    if saldo_compra == 0 and tot_sc_aberto == 0:
      st.success(
          "🎉 Tudo certo com Compras! Todos os itens comprados foram 100%"
          " entregues e não há SCs em aberto."
      )
    else:
      if not df_comp_trabalho.empty:
        df_comp_pend = df_comp_trabalho[
            df_comp_trabalho["Saldo_Falta_Entregar"] > 0
        ]
        if not df_comp_pend.empty:
          with st.expander(
              f"🚚 **Ordens de Compra Pendentes de Entrega ({len(df_comp_pend)}"
              f" itens | {saldo_compra:,} pçs)**",
              expanded=True,
          ):
            for _, rc in df_comp_pend.iterrows():
              st.markdown(
                  f"• **`{rc['COD_PECA']}`** ({rc['Descricao'][:35]}): Falta"
                  f" **{int(rc['Saldo_Falta_Entregar'])} pçs** | Fornecedor:"
                  f" `{rc['Fornecedor']}` | Previsão: `{rc['Data_Fornecedor']}`"
              )

      if not df_sc_trabalho.empty:
        with st.expander(
            f"📋 **Solicitações de Compra em Aberto ({qtd_itens_sc} itens |"
            f" {tot_sc_aberto:,} pçs)**",
            expanded=True,
        ):
          for _, rsc in df_sc_trabalho.iterrows():
            st.markdown(
                f"• **`{rsc['COD_PECA']}`** ({rsc['Descricao'][:35]}):"
                f" **{int(rsc['Qtd_SC'])} pçs** | SC Nº: `{rsc['Num_SC']}` |"
                f" Solicitante: `{rsc['Solicitante']}`"
            )
  else:
    st.info("Nenhuma ordem de compra cadastrada para este lote.")

# ==============================================================================
# 3. ABA BALANÇO COMPLETO METÁLICOS (STATUS E DATAS NO INÍCIO)
# ==============================================================================
with tab_metalicos:
  if not df_trabalho.empty:
    ...
    df_view_balanco = df_trabalho[colunas_balanco_ordem].rename(columns={
    ...
    with c_tit:
      st.markdown(
          f"**Balanço Metálico ({len(df_trabalho)} itens) —"
          f" {projeto_ativo_nome}**"
      )
    with c_btns:
      cb1, cb2 = st.columns(2)
      with cb1:
        renderizar_botao_copiar_tabela(df_view_balanco, "btn_cp_balanco")
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_trabalho, "Balanco_Metalicos"),
            file_name="Balanco_Completo_Metalicos.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    st.dataframe(
        df_view_balanco, use_container_width=True, hide_index=True, height=650
    )
  else:
    st.info("Nenhum dado metálico encontrado para o filtro selecionado.")

# ==============================================================================
# 4. ABA METÁLICOS TOTAL (STATUS E DATAS NO INÍCIO)
# ==============================================================================
  ...
  if lista_dfs_total:
    ...
    with c_origem_tot:
      sel_origem = st.multiselect(
          ...
          ["🏭 Fabricação Interna", "📦 Compra / Externado"],
          default=["🏭 Fabricação Interna", "📦 Compra / Externado"],
      )

    if sel_origem:
      df_unificado_total = df_unificado_total[
          df_unificado_total["Origem_Tipo"].isin(sel_origem)
      ]
    ...
    colunas_total_ordem = [
        ...
        "Qtd_Total_Demanda",
        "Qtd_Entregue_Pronta",
        "Saldo_Pendente_Entrega",
        "Falta_Produzir_Interno",
        "Doc_Origem_Envio",
        "NF_Retorno_Entrega",
    ]
    ...
    for col_chk in colunas_total_ordem:
      if col_chk not in df_unificado_total.columns:
        df_unificado_total[col_chk] = (
            0.0
            if "Qtd" in col_chk or "Saldo" in col_chk or "Falta" in col_chk
            else "-"
        )

    df_view_total = df_unificado_total[colunas_total_ordem].rename(columns={
        "Status_Unificado": "Status do Item",
    ...
    with c_tit_tot:
      st.subheader(
          f"📊 Metálicos Total ({len(df_unificado_total)} itens |"
          f" {tot_pronto_geral:,} de {tot_demanda_geral:,} entregues/prontos)"
      )
    with c_btns_tot:
      cb1, cb2 = st.columns(2)
      with cb1:
      ...
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_view_total, "Metalicos_Total_Geral"),
            file_name="Metalicos_Total_Unificado.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    st.dataframe(
        df_view_total, use_container_width=True, hide_index=True, height=650
    )
  else:
    st.info(
        "Nenhum registro de fabricação ou compra encontrado para os filtros"
        " selecionados."
    )

# ==============================================================================
# 5. ABA PEÇAS FABRICADAS (STATUS E DATAS NO INÍCIO)
# ==============================================================================
with tab_fabricadas:
  if not df_trabalho.empty:
    ...
    if not df_fab_feitas.empty:
      ...
      df_view_fab = df_fab_feitas[cols_fab_feitas].rename(columns={
          "Status": "Status Atual",
          "Data_Fabricacao": "Data Conclusão Fabricação",
          "OBS_NORM": "Observação (Lote)",
          "COD_PECA": "Código Peça",
          "Descricao": "Descrição",
          "Qtd_OP": "Programado OP",
          "Qtd_Fabr": "Total Já Fabricado",
          "Env_Pintura": "Já Saiu p/ Pintura",
          "Aguardando_Envio": "Saldo no Estoque Fábrica",
      })

      c_tit, c_btns = st.columns([3, 2])
      with c_tit:
        st.subheader(
            f"🏭 Peças Já Fabricadas ({len(df_fab_feitas)} itens | {tot_fab:,}"
            " peças)"
        )
      with c_btns:
        cb1, cb2 = st.columns(2)
        ...
        with cb2:
          st.download_button(
              "📥 Exportar (.xlsx)",
              data=gerar_excel_tabela(df_fab_feitas, "Pecas_Fabricadas"),
              file_name="Pecas_Fabricadas.xlsx",
              mime=(
                  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              ),
          )
      ...
      st.dataframe(
          df_view_fab, use_container_width=True, hide_index=True, height=650
      )
    else:
...
# ==============================================================================
# 6. ABA PEÇAS RETORNADAS (STATUS E DATAS NO INÍCIO)
# ==============================================================================
with tab_retornadas:
  if not df_trabalho.empty:
    df_p2 = df_trabalho[df_trabalho["Ret_Pintura"] > 0].copy()
    forns_ret = sorted(
        [f for f in df_p2["Fornecedor_Tratamento"].unique() if f != "-"]
    )
    if forns_ret:
      sel_forn_ret = st.multiselect(
          "Filtrar por Fornecedor (Quem entregou):", forns_ret, default=[]
      )
      if sel_forn_ret:
    ...
    if not df_p2.empty:
      ...
      with c_tit:
        st.subheader(
            f"✅ Peças que já Retornaram ({len(df_p2)} itens | {tot_ret:,} peças)"
        )
      with c_btns:
        cb1, cb2 = st.columns(2)
        with cb1:
        ...
        with cb2:
          st.download_button(
              "📥 Exportar (.xlsx)",
              data=gerar_excel_tabela(df_p2, "Pecas_Retornadas"),
              file_name="Pecas_Retornadas.xlsx",
              mime=(
                  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              ),
          )
      ...
      st.dataframe(
          df_view_ret, use_container_width=True, hide_index=True, height=650
      )
    else:
...
with tab_pend_trat:
  if not df_trabalho.empty:
    ...
    forns_pend = sorted(
        [f for f in df_p1["Fornecedor_Tratamento"].unique() if f != "-"]
    )
    if forns_pend:
      sel_forn_pend = st.multiselect(
          "Filtrar por Fornecedor (Onde a peça está):", forns_pend, default=[]
      )
      if sel_forn_pend:
    ...
    if not df_p1.empty:
      ...
      with c_tit:
        st.subheader(
            "🚨 Falta Retorno de Tratamento Externo ("
            f"{len(df_p1)} itens | {saldo_rua:,} peças)"
        )
      with c_btns:
        cb1, cb2 = st.columns(2)
        with cb1:
        ...
        with cb2:
          st.download_button(
              "📥 Exportar (.xlsx)",
              data=gerar_excel_tabela(df_p1, "Falta_Retorno_Tratamento"),
              file_name="Falta_Retorno_Tratamento.xlsx",
              mime=(
                  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              ),
          )
      ...
      st.success(
          "🎉 Nenhuma peça pendente de retorno de tratamento para este filtro!"
      )
...
with tab_aguard_envio:
  if not df_trabalho.empty:
    ...
    if not df_p3.empty:
      ...
      with c_tit:
        st.subheader(
            "🚚 Peças Fabricadas Aguardando Envio ("
            f"{len(df_p3)} itens | {falta_env:,} peças)"
        )
      with c_btns:
        cb1, cb2 = st.columns(2)
        with cb1:
        ...
        with cb2:
          st.download_button(
              "📥 Exportar (.xlsx)",
              data=gerar_excel_tabela(df_p3, "Aguardando_Envio"),
              file_name="Fabricadas_Aguardando_Envio.xlsx",
              mime=(
                  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              ),
          )
      ...
      st.dataframe(
          df_view_aguard, use_container_width=True, hide_index=True, height=650
      )
    else:
      if not sel_obs_global:
        st.success(
            "🎉 Todas as peças fabricadas já foram enviadas para tratamento!"
        )
      else:
  ...
  if not df_trabalho.empty:
    ...
    if not df_p4.empty:
      ...
      df_view_falta_fab = df_p4[cols_fab].rename(columns={
          ...
          "Qtd_OP": "Programado OP",
          "Qtd_Fabr": "Já Fabricado",
          "Falta_Fabricar": "Saldo a Produzir",
      })

      c_tit, c_btns = st.columns([3, 2])
      with c_tit:
        st.subheader(
            f"⚙️ Peças que Faltam Ser Fabricadas ({len(df_p4)} itens |"
            f" {falta_fab:,} peças)"
        )
      with c_btns:
        cb1, cb2 = st.columns(2)
        ...
        with cb2:
          st.download_button(
              "📥 Exportar (.xlsx)",
              data=gerar_excel_tabela(df_p4, "Falta_Fabricar"),
              file_name="Falta_Fabricar_Internamente.xlsx",
              mime=(
                  "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
              ),
          )
      ...
      if not sel_obs_global:
        st.success(
            "🎉 100% da programação de fábrica já foi produzida internamente!"
        )
      else:
        st.info(
            "Nenhuma peça pendente de fabricação interna para o projeto"
            " selecionado."
        )

# ==============================================================================
# 10. ABA COMPRAS EXTERNAS (STATUS E DATAS NO INÍCIO)
# ==============================================================================
with tab_compras:
  if not df_comp_trabalho.empty:
    ...
    with col_forn_f:
      fornecedores = sorted([
          str(f)
          for f in df_comp_trabalho["Fornecedor"].unique()
          if str(f).strip() and str(f) != "-"
      ])
      sel_forn = st.multiselect(
          "Filtrar por Fornecedor de Compra:", fornecedores, default=[]
      )
    with col_st_f:
      status_comp = sorted([
          str(s)
          for s in df_comp_trabalho["Status_Compra"].unique()
          if str(s).strip() and str(s) != "-"
      ])
      sel_st_comp = st.multiselect(
          "Filtrar por Status de Entrega:", status_comp, default=[]
      )
    ...
    if sel_st_comp:
      df_comp_view = df_comp_view[
          df_comp_view["Status_Compra"].isin(sel_st_comp)
      ]
    ...
    colunas_comp = [
        ...
        "Qtd_Comprada",
        "Qtd_Entregue",
        "Saldo_Falta_Entregar",
        "NF_Entrega",
    ]
    df_view_compras = df_comp_view[colunas_comp].rename(columns={
        "Status_Compra": "Status",
        "Data_Entrega": "DT Ent.",
        "Data_Fornecedor": "Data Fornecedor",
        "OBS_NORM": "Observação (L)",
        "COD_PECA": "Código Peça",
        "Descricao": "Descrição",
        "Fornecedor": "Fornecedor",
        "Qtd_Comprada": "QT Comprada",
        "Qtd_Entregue": "QTD Entregue",
        "Saldo_Falta_Entregar": "FAL",
        "NF_Entrega": "NF Ent.",
    })
    ...
    with c_tit:
      st.subheader(
          "📦 Compras e Projetos Externados ("
          f"{len(df_comp_trabalho)} itens | {tot_entregue:,} de"
          f" {tot_comprado:,} entregues)"
      )
    with c_btns:
      cb1, cb2 = st.columns(2)
      with cb1:
      ...
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_comp_trabalho, "Compras_Externas"),
            file_name="Compras_Projetos_Externados.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    st.dataframe(
        df_view_compras, use_container_width=True, hide_index=True, height=650
    )
  else:
...
with tab_sc:
  if not df_sc_trabalho.empty:
    ...
    with col_solic_f:
      solicitantes = sorted([
          str(s)
          for s in df_sc_trabalho["Solicitante"].unique()
          if str(s).strip() and str(s) != "-"
      ])
      sel_solic = st.multiselect(
          "Filtrar por Solicitante:", solicitantes, default=[]
      )
    with col_ped_f:
      pedidos_sc = sorted([
          str(p)
          for p in df_sc_trabalho["Pedido"].unique()
          if str(p).strip() and str(p) != "-"
      ])
      sel_ped_sc = st.multiselect(
          "Filtrar por Pedido Vinculado:", pedidos_sc, default=[]
      )
    ...
    df_view_sc = df_sc_view[colunas_sc].rename(columns={
        ...
        "Emissao": "Emissão",
        "Filial": "Filial",
        "Num_SC": "Nº Solicitação",
        "Item": "Item",
        "COD_PECA": "Produto",
        "UM": "UM",
        "Descricao": "Descrição",
        "Qtd_SC": "Qtde da SC",
        "OBS_NORM": "Observação (I)",
        "Pedido": "Pedido",
    ...
    with c_tit:
      st.subheader(
          "📋 Solicitações de Compras em Aberto ("
          f"{len(df_sc_trabalho)} itens | {tot_sc_aberto:,} peças)"
      )
    with c_btns:
      cb1, cb2 = st.columns(2)
      with cb1:
      ...
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_sc_trabalho, "SC_Em_Aberto"),
            file_name="Solicitacoes_Compras_Aberto.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    st.dataframe(
        df_view_sc, use_container_width=True, hide_index=True, height=650
    )
  else:
    st.info(
        "Nenhuma Solicitação de Compras em aberto encontrada para o filtro"
        " selecionado."
    )
  ...
  if not df_op_raw.empty:
    ...
    with c_btn_raw:
      ...
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_op_raw, "Base_OP"),
            file_name="Base_OP_Fabricacao.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    if busca_cod:
      st.dataframe(
          df_op_raw[
              df_op_raw["Produto"]
              .astype(str)
              .str.contains(busca_cod, case=False, na=False)
          ],
          use_container_width=True,
          hide_index=True,
          height=650,
      ...
      st.dataframe(
          df_op_raw, use_container_width=True, hide_index=True, height=650
      )
  ...
  if not df_rom_raw.empty:
    ...
    with c_btn_raw:
      ...
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_rom_raw, "Base_Romaneio"),
            file_name="Base_Romaneio_Pintura.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    if busca_cod:
      ...
      st.dataframe(
          df_rom_raw, use_container_width=True, hide_index=True, height=650
      )
  ...
  if not df_comp_raw.empty:
    ...
    with c_btn_raw:
      ...
      with cb2:
        st.download_button(
            "📥 Exportar (.xlsx)",
            data=gerar_excel_tabela(df_comp_raw, "Base_Compras"),
            file_name="Base_Compras_Completa.xlsx",
            mime=(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            ),
        )
    ...
    if busca_cod:
      col_busca_prod = [
          c
          for c in df_comp_raw.columns
          if "PROD" in str(c).upper() or "COD" in str(c).upper()
      ]
      col_target = (
          col_busca_prod[0] if col_busca_prod else df_comp_raw.columns[2]
      )
      st.dataframe(
          df_comp_raw[
              df_comp_raw[col_target]
              .astype(str)
              .str.contains(busca_cod, case=False, na=False)
          ],
          use_container_width=True,
      ...
      st.dataframe(
          df_comp_raw, use_container_width=True, hide_index=True, height=650
      )
