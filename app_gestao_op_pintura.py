...
def processar_todas_as_bases(mtimes, pasta_base=STORAGE_DIR):
    ...
    # 1. OP (Busca por coluna de Observação oficial / Coluna L)
    df_op = pd.DataFrame()
    ...
    # 5. Cruzamento Robusto com Preservação de Valores Reais
    def format_unique_join(x):
        vals = [str(v) for v in x if str(v) not in ["-", "", "nan", "None"]]
        return ", ".join(sorted(set(vals))) or "-"

    # Dicionário da OP para reconciliação quando a observação tiver leves variações
    mapa_op_exata = {}
    mapa_plan_exata = {}
    mapa_op_tat = {}
    mapa_plan_tat = {}
    ...
    if not df_op.empty:
        for _, r in df_op.iterrows():
            ...
            mapa_op_exata[(obs, peca)] = mapa_op_exata.get((obs, peca), 0.0) + r["QTD_PROD"]
            mapa_plan_exata[(obs, peca)] = mapa_plan_exata.get((obs, peca), 0.0) + r["QTD_PLAN"]
            
            if tat and tat != "-":
                mapa_op_tat[(tat, peca)] = mapa_op_tat.get((tat, peca), 0.0) + r["QTD_PROD"]
    ...
    if not df_cruz_obs.empty:
        ...
        # Se a linha veio do Romaneio e Qtd_Fabr / Qtd_OP estiver 0, busca na OP pelo TAT Base do projeto
        def reconciliar_fabricado_op(r):
            q_fab = r["Qtd_Fabr"]
            if q_fab > 0: return q_fab
            obs = str(r["OBS_NORM"]).strip()
            tat = str(r["TAT_BASE"]).strip()
            peca = str(r["COD_PECA"]).strip()
            if (obs, peca) in mapa_op_exata: return mapa_op_exata[(obs, peca)]
            if (tat, peca) in mapa_op_tat: return mapa_op_tat[(tat, peca)]
            return 0.0

        def reconciliar_programado_op(r):
            q_op = r["Qtd_OP"]
            if q_op > 0: return q_op
            obs = str(r["OBS_NORM"]).strip()
            tat = str(r["TAT_BASE"]).strip()
            peca = str(r["COD_PECA"]).strip()
            if (obs, peca) in mapa_plan_exata: return mapa_plan_exata[(obs, peca)]
            if (tat, peca) in mapa_plan_tat: return mapa_plan_tat[(tat, peca)]
            return 0.0

        df_cruz_obs["Qtd_Fabr"] = df_cruz_obs.apply(reconciliar_fabricado_op, axis=1)
        df_cruz_obs["Qtd_OP"] = df_cruz_obs.apply(reconciliar_programado_op, axis=1)
        ...
        def calc_status(r):
            if r["Falta_Fabricar"] > 0: return "1. Falta Fabricar Internamente"
...
with col_multisel_obs:
    sel_obs_global = st.multiselect(
        ...
        placeholder="Digite parte do nome ou código (Ex: 11570, 11761, COROLLA, RANGER)..."
    )
...
if sel_obs_global:
    # 1. Filtro Fábrica / Romaneio
    if not df_trabalho.empty:
        cond_obs = df_trabalho["OBS_NORM"].isin(sel_obs_global) if "OBS_NORM" in df_trabalho.columns else False
        tats_sel = [extrair_tat_base(o) for o in sel_obs_global if extrair_tat_base(o)]
        cond_tat = df_trabalho["TAT_BASE"].isin(tats_sel) if "TAT_BASE" in df_trabalho.columns else False
        df_trabalho = df_trabalho[cond_obs | cond_tat]
    
    # 2. Filtro Compras
    if not df_comp_trabalho.empty:
        cond_obs_c = df_comp_trabalho["OBS_NORM"].isin(sel_obs_global) if "OBS_NORM" in df_comp_trabalho.columns else False
        cond_tat_c = df_comp_trabalho["TAT_BASE"].isin(tats_sel) if "TAT_BASE" in df_comp_trabalho.columns else False
        df_comp_trabalho = df_comp_trabalho[cond_obs_c | cond_tat_c]
        
    # 3. Filtro SC
    if not df_sc_trabalho.empty:
        cond_obs_s = df_sc_trabalho["OBS_NORM"].isin(sel_obs_global) if "OBS_NORM" in df_sc_trabalho.columns else False
        cond_tat_s = df_sc_trabalho["TAT_BASE"].isin(tats_sel) if "TAT_BASE" in df_sc_trabalho.columns else False
        df_sc_trabalho = df_sc_trabalho[cond_obs_s | cond_tat_s]
...
with tab_mensal:
    ...
    if not df_op.empty:
        ...
        if sel_obs_global:
            tats_sel = [extrair_tat_base(o) for o in sel_obs_global if extrair_tat_base(o)]
            df_op_filtrada_data = df_op_filtrada_data[df_op_filtrada_data["OBS_NORM"].isin(sel_obs_global) | df_op_filtrada_data["TAT_BASE"].isin(tats_sel)]
        ...
        with col_t_mes:
            ...
            with c_m2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_mes_view, "Producao_Mensal_Consolidada"), file_name="Producao_Mensal_Consolidada.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_metalicos:
    if not df_trabalho.empty:
        ...
        with c_btns:
            ...
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_trabalho, "Balanco_Metalicos"), file_name="Balanco_Completo_Metalicos.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

        st.dataframe(df_view_balanco, use_container_width=True, hide_index=True, height=650)
...
with tab_fabricadas:
    if not df_trabalho.empty:
        ...
        if not df_fab_feitas.empty:
            cols_fab_feitas = [
                "OBS_NORM", "COD_PECA", "Descricao", "Qtd_OP", "Qtd_Fabr", 
                "Env_Pintura", "Aguardando_Envio", "Data_Fabricacao", "Status"
            ]
            ...
            c_tit, c_btns = st.columns([3, 2])
            with c_tit: st.subheader(f"🏭 Peças Já Fabricadas ({len(df_fab_feitas)} itens | {tot_fab:,} peças)")
            with c_btns:
                cb1, cb2 = st.columns(2)
                with cb1:
                    renderizar_botao_copiar_tabela(df_view_fab, "btn_cp_fabricadas")
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_fab_feitas, "Pecas_Fabricadas"), file_name="Pecas_Fabricadas.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if not df_trabalho.empty:
        ...
        if not df_p2.empty:
            ...
            with c_btns:
                ...
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p2, "Pecas_Retornadas"), file_name="Pecas_Retornadas.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if not df_trabalho.empty:
        ...
        if not df_p1.empty:
            ...
            with c_btns:
                ...
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p1, "Falta_Retorno_Tratamento"), file_name="Falta_Retorno_Tratamento.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_aguard_envio:
    if not df_trabalho.empty:
        ...
        if not df_p3.empty:
            cols_env = [
                "OBS_NORM", "COD_PECA", "Descricao", "Qtd_OP", "Qtd_Fabr", 
                "Env_Pintura", "Aguardando_Envio", "Data_Fabricacao"
            ]
            ...
            with c_btns:
                ...
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p3, "Aguardando_Envio"), file_name="Fabricadas_Aguardando_Envio.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if not df_trabalho.empty:
        ...
        if not df_p4.empty:
            ...
            with c_btns:
                ...
                with cb2:
                    st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_p4, "Falta_Fabricar"), file_name="Falta_Fabricar_Internamente.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_compras:
    if not df_comp_trabalho.empty:
        ...
        with c_btns:
            ...
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_comp_trabalho, "Compras_Externas"), file_name="Compras_Projetos_Externados.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_sc:
    if not df_sc_trabalho.empty:
        ...
        colunas_sc = [
            "Filial", "Num_SC", "Item", "COD_PECA", "UM", "Descricao", 
            "Qtd_SC", "Necessidade", "OBS_NORM", "Emissao", "Pedido", "Solicitante", "Classe_Valor"
        ]
        ...
        with c_btns:
            ...
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_sc_trabalho, "SC_Em_Aberto"), file_name="Solicitacoes_Compras_Aberto.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_base_op:
    ...
    if not df_op_raw.empty:
        ...
        with c_tit_raw: st.caption(f"Total de registros na base: {len(df_op_raw):,}")
        with c_btn_raw:
            cb1, cb2 = st.columns(2)
            with cb1:
                renderizar_botao_copiar_tabela(df_op_raw, "btn_cp_base_op")
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_op_raw, "Base_OP"), file_name="Base_OP_Fabricacao.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_base_rom:
    ...
    if not df_rom_raw.empty:
        c_tit_raw, c_btn_raw = st.columns([3, 2])
        with c_tit_raw: st.caption(f"Total de registros na base: {len(df_rom_raw):,}")
        with c_btn_raw:
            cb1, cb2 = st.columns(2)
            with cb1:
                renderizar_botao_copiar_tabela(df_rom_raw, "btn_cp_base_rom")
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_rom_raw, "Base_Romaneio"), file_name="Base_Romaneio_Pintura.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab_base_comp:
    ...
    if not df_comp_raw.empty:
        c_tit_raw, c_btn_raw = st.columns([3, 2])
        with c_tit_raw: st.caption(f"Total de registros na base: {len(df_comp_raw):,}")
        with c_btn_raw:
            cb1, cb2 = st.columns(2)
            with cb1:
                renderizar_botao_copiar_tabela(df_comp_raw, "btn_cp_base_comp")
            with cb2:
                st.download_button("📥 Exportar (.xlsx)", data=gerar_excel_tabela(df_comp_raw, "Base_Compras"), file_name="Base_Compras_Completa.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
