...
def processar_todas_as_bases(mtimes, pasta_base=STORAGE_DIR):
  ...
  if not df_op_raw.empty:
    col_obs_op = (
        df_op_raw.columns[1]
        if len(df_op_raw.columns) >= 2
        and "OBSERV" in normalizar_texto(df_op_raw.columns[1])
        else buscar_col_flex(
            df_op_raw,
            ["OBSERVAÇÃO", "OBSERVAÇÕES", "OBSERVACAO", "OBSERVACOES", "OBS"],
        )
    )
    if not col_obs_op and len(df_op_raw.columns) >= 12:
  ...
  if not df_rom_raw.empty:
    col_obs_rom = buscar_col_flex(
        df_rom_raw,
        ["OBSERVAÇÕES", "OBSERVACOES", "OBSERVAÇÃO", "OBSERVACAO", "OBS"],
    )
  ...
  if not df_cruz_obs.empty:
    ...
    def calc_status(r):
      if r["Falta_Fabricar"] > 0:
        return "1. Falta Fabricar Internamente"
      elif r["Aguardando_Envio"] > 0 and r["Env_Pintura"] == 0:
        return "2. Fabricado (Não Enviado)"
      elif r["Saldo_Pendente_Pintura"] > 0:
        return "3. Em Tratamento Externo"
      elif r["Ret_Pintura"] >= r["Qtd_Fabr"] and r["Qtd_Fabr"] > 0:
        return "4. 100% Concluído (Entregue)"
      else:
        return "5. Parcial / Diversos"
...
with col_multisel_obs:
  sel_obs_global = st.multiselect(
      ...
      placeholder=(
          "Digite parte do nome ou código (Ex: 11570, 11761, COROLLA,"
          " RANGER)..."
      ),
  )
...
with tab_mensal:
  ...
  if not df_op.empty:
    ...
    with col_t_mes:
      ...
      st.download_button(
          "📥 Exportar Relatório Mensal (.xlsx)",
...
with tab_fabricadas:
  if not df_trabalho.empty:
    ...
    with c_btn:
      st.download_button(
          "📥 Exportar Peças Fabricadas (.xlsx)",
    ...
    if not df_fab_feitas.empty:
      ...
      st.dataframe(
          df_fab_feitas[cols_fab_feitas].rename(columns={
              "OBS_NORM": "Observação (Lote)",
...
with tab_retornadas:
  if not df_trabalho.empty:
    ...
    with c_btn:
      st.download_button(
          "📥 Exportar Peças Retornadas (.xlsx)",
...
with tab_pend_trat:
  if not df_trabalho.empty:
    ...
    with c_tit:
      st.subheader(
          f"🚨 Falta Retorno de Tratamento Externo ({len(df_p1)} itens |"
          f" {saldo_rua:,} peças)"
      )
    with c_btn:
      st.download_button(
          "📥 Exportar Falta Retorno (.xlsx)",
...
with tab_aguard_envio:
  if not df_trabalho.empty:
    ...
    with c_btn:
      st.download_button(
          "📥 Exportar Aguardando Envio (.xlsx)",
...
# ==============================================================================
# 8. ABA FALTA FABRICAR (MOSTRA AS PEÇAS COM SALDO A PRODUZIR)
# ==============================================================================
with tab_falta_fab:
  if not df_trabalho.empty:
...
with tab_compras:
  if not df_comp_trabalho.empty:
    ...
    with c_btn:
      st.download_button(
          "📥 Exportar Compras Filtradas (.xlsx)",
    ...
    st.dataframe(
        df_comp_view[colunas_comp].rename(columns={
