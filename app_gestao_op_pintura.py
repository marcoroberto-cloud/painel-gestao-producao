...
# Componente HTML / JS para Copiar Tabela para a Área de Transferência
...
def processar_todas_as_bases(mtimes, pasta_base=STORAGE_DIR):
    ...
    if not df_op_raw.empty:
        col_obs_op = df_op_raw.columns[1] if (len(df_op_raw.columns) >= 2 and "OBSERV" in normalizar_texto(df_op_raw.columns[1])) else buscar_col_flex(df_op_raw, ["OBSERVAÇÃO", "OBSERVAÇÕES", "OBSERVACAO", "OBSERVACOES", "OBS"])
